...
def _write_q1_main(sheet, rows: list[dict]) -> None:
    if sheet.max_column < 10 or sheet.cell(1, 8).value != "作业时间（s）":
        sheet.insert_cols(8, 1)
    headers = [
        "架次编号", "服务区编号", "机型编号", "货箱编号列表", "总质量（kg）", "总体积（m³）",
        "往返时间（s）", "作业时间（s）", "架次能耗（kWh）", "返航SOC（%）",
    ]
    ...
    for row_idx, item in enumerate(rows, start=2):
        values = [
            ...
            item["mass_kg"], item["volume_m3"], item["time_s"], item["time_s"],
            item["energy_kwh"], item["soc_percent"],
        ]
        ...
        sheet.cell(row_idx, 10).number_format = "0.00"
...
def _write_detail_result(sheet, rows: list[dict]) -> None:
    if sheet.max_column < 11 or sheet.cell(1, 8).value != "作业时间（s）":
        sheet.insert_cols(8, 1)
    headers = [
        "架次编号", "服务区编号", "机型编号", "货箱编号列表", "总质量（kg）", "总体积（m³）",
        "往返时间（s）", "作业时间（s）", "架次能耗（kWh）", "返航SOC（%）", "箱数",
    ]
    for col, header in enumerate(headers, start=1):
        _copy_style(sheet.cell(1, 7), sheet.cell(1, col))
        sheet.cell(1, col, header)
    ...
    for row_idx, item in enumerate(rows, start=2):
        values = [
            item["batch_id"], item["area_id"], item["uav_type"], item["box_ids_normalized"],
            item["mass_kg"], item["volume_m3"], item["time_s"], item["time_s"],
            item["energy_kwh"], item["soc_percent"], item["box_count"],
        ]
        for col, value in enumerate(values, start=1):
            _copy_style(sheet.cell(2, col), sheet.cell(row_idx, col))
            sheet.cell(row_idx, col, value)
        ...
        for col in (5, 6, 7, 8, 9):
            sheet.cell(row_idx, col).number_format = "0.00000000"
        sheet.cell(row_idx, 10).number_format = "0.00"
        sheet.cell(row_idx, 11).number_format = "0"
    sheet.freeze_panes = "A2"
    sheet.auto_filter.ref = f"A1:K{len(rows) + 1}"
    sheet.sheet_view.showGridLines = False
    for col, width in {
        "A": 22, "B": 12, "C": 10, "D": 72, "E": 16, "F": 16,
        "G": 18, "H": 18, "I": 20, "J": 16, "K": 10,
    }.items():
        sheet.column_dimensions[col].width = width
...
def replace_q1_submission(
    ...
    notes.cell(
        row_idx,
        2,
        "已替换为统一模型重算的 18 架次方案；新增“作业时间（s）”列。"
        "每架次为 O01→S_i→O01 单点往返，因此作业时间等于往返时间。",
    )
